/* Shared header, progress tracking (localStorage), and dashboard rendering
   for the AI Engineer Course. Same local-first design as DSA_tool: localStorage
   is the always-available source of truth; Supabase sync is optional and
   additive for signed-in users (tables ai_progress / ai_notes). */

const PROGRESS_KEY = 'ai-progress-v1';
...
async function syncModuleToCloud(id, done) {
  ...
  if (!session) return;
  try {
    await sb.from('ai_progress').upsert({
    ...
    console.warn('ai_progress sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeProgress() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('ai_progress').select('module_id, completed').eq('completed', true);
  if (error) {
    console.warn('ai_progress pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('ai:progress-synced'));
}

/* Per-lesson free-text notes — newer updated_at wins on merge. */
const NOTES_KEY = 'ai-notes-v1';
...
async function syncNoteToCloud(id, text, updated_at) {
  ...
  try {
    ...
    console.warn('ai_notes sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeNotes() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('ai_notes').select('module_id, note, updated_at');
  if (error) {
    console.warn('ai_notes pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('ai:notes-synced'));
}
  ...
  el.innerHTML = `
    <header class="site">
      <div class="bar">
        ...
        <a class="brand" href="index.html">AI Engineer <span>Course</span></a>
        <nav>${nav}</nav>
