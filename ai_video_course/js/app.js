/* Shared header, progress tracking (localStorage), depth-mode toggle, and
   dashboard rendering for the AI Video Course. Same local-first design as
   ...
   (tables aivideo_progress / aivideo_notes). */

const PROGRESS_KEY = 'aivideo-progress-v1';
...
async function syncModuleToCloud(id, done) {
  ...
  if (!session) return;
  try {
    await sb.from('aivideo_progress').upsert({
    ...
    console.warn('aivideo_progress sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeProgress() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('aivideo_progress').select('module_id, completed').eq('completed', true);
  if (error) {
    console.warn('aivideo_progress pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('aivideo:progress-synced'));
}

/* Per-lesson free-text notes — newer updated_at wins on merge. */
const NOTES_KEY = 'aivideo-notes-v1';
...
async function syncNoteToCloud(id, text, updated_at) {
  ...
  if (!session) return;
  try {
    await sb.from('aivideo_notes').upsert({
      user_id: session.user.id,
      module_id: id,
      note: text,
      updated_at,
    ...
    console.warn('aivideo_notes sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeNotes() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('aivideo_notes').select('module_id, note, updated_at');
  if (error) {
    console.warn('aivideo_notes pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('aivideo:notes-synced'));
}
...
const DEPTH_KEY = 'aivideo-depth-v1';
...
function setDepthMode(mode) {
  ...
  window.dispatchEvent(new CustomEvent('aivideo:depth-changed', { detail: { mode: getDepthMode() } }));
}
...
function buildHeader(active) {
  ...
  el.innerHTML = `
    <header class="site">
      <div class="bar">
        <a class="brand" href="index.html">AI Video <span>Course</span></a>
        <nav>${nav}</nav>
