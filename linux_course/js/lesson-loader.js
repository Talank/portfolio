...
Lesson schema (window.LESSONS[id]):
  ...
  tech       : [{q, a}]                          — "Technicality corner": why this command/
               tool/mechanism exists, what's under the hood
  code       : {title, intro, code, notes[]}     — worked example to read (usually a
               terminal transcript or shell script)
...
function renderLesson(id) {
  ...
  document.title = `${data.title} — Linux Course`;
  ...
  const techHtml = (data.tech || []).length ? `
    ...
    <p class="lede" style="font-size:0.92rem;">The questions you'd be embarrassed to ask out loud, answered properly: why the command exists, what it really does, where the mechanism comes from.</p>
    ${data.tech.map(t => `
      <details class="tech">
  ...
  root.innerHTML = `
    ...
    ${techHtml}
    ${codeHtml}
    ${data.lab ? '<h2>Terminal Lab — prove it in the shell</h2><div id="lab-container"></div>' : ''}
  ...
  if (data.deepDive) {
    ...
    window.addEventListener('linux:depth-changed', () => {
      renderDeepDive(document.getElementById('deepdive-container'), data.deepDive);
    });
  }
  ...
  window.addEventListener('linux:progress-synced', () => { check.checked = isComplete(id); });
  ...
  window.addEventListener('linux:notes-synced', () => {
