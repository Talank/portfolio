...
function renderLesson(id) {
  ...
  root.innerHTML = `
    ...
    ${techHtml}
    ${codeHtml}
    ${data.lab ? '<h2>Practice Lab — prove you know the keys</h2><div id="lab-container"></div>' : ''}
  ...
  if (data.deepDive) {
    ...
    window.addEventListener('emacs:depth-changed', () => {
      renderDeepDive(document.getElementById('deepdive-container'), data.deepDive);
    });
  }
  ...
  window.addEventListener('emacs:progress-synced', () => { check.checked = isComplete(id); });
  ...
  window.addEventListener('emacs:notes-synced', () => {
