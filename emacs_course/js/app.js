/* Shared header, progress tracking (localStorage), depth-mode toggle, and
   dashboard rendering for the Emacs Course. Same local-first design as
   ...
   (tables emacs_progress / emacs_notes). */

const PROGRESS_KEY = 'emacs-progress-v1';
...
async function syncModuleToCloud(id, done) {
  ...
  if (!session) return;
  try {
    await sb.from('emacs_progress').upsert({
    ...
    console.warn('emacs_progress sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeProgress() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('emacs_progress').select('module_id, completed').eq('completed', true);
  if (error) {
    console.warn('emacs_progress pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('emacs:progress-synced'));
}

/* Per-lesson free-text notes — newer updated_at wins on merge. */
const NOTES_KEY = 'emacs-notes-v1';
...
async function syncNoteToCloud(id, text, updated_at) {
  ...
  if (!session) return;
  try {
    await sb.from('emacs_notes').upsert({
      user_id: session.user.id,
      module_id: id,
      note: text,
      updated_at,
    ...
    console.warn('emacs_notes sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeNotes() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('emacs_notes').select('module_id, note, updated_at');
  if (error) {
    console.warn('emacs_notes pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('emacs:notes-synced'));
}
...
const DEPTH_KEY = 'emacs-depth-v1';
...
function setDepthMode(mode) {
  ...
  window.dispatchEvent(new CustomEvent('emacs:depth-changed', { detail: { mode: getDepthMode() } }));
}
...
function buildHeader(active) {
  ...
  el.innerHTML = `
    <header class="site">
      <div class="bar">
        <a class="brand" href="index.html">Emacs <span>Course</span></a>
        <nav>${nav}</nav>
