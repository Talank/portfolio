...
# voyage ships in Nepali and in English: same story, same chapter order, same
# @algo blocks, different language and a different cast — so they need separate
# sidecars, separate audio and separate manifests, but they are otherwise the
# same build. These three name this edition's outputs; the English profile
# (build_bedtime_en.py) rebinds them and nothing else about the sidecars moves.
#
# Durations differ between editions, so the manifests genuinely cannot be
# shared: English runs shorter than Nepali for the same paragraph, which moves
# every chapter offset and every tier size.
MANIFEST_STEM = "manifest"          # -> manifest.json, manifest.js
CHAPTERS_TXT = "chapters.txt"
MANIFEST_GLOBAL = "BEDTIME_MANIFEST"    # the window.* the player reads
AUDIO_URL = "data/bedtime/audio/"       # where the page fetches segments from
...
def prior_single_file():
    ...
    forward instead.
    """
    path = os.path.join(HERE, MANIFEST_STEM + ".json")
    try:
        with open(path, encoding="utf-8") as f:
            old = json.load(f)
    except (OSError, ValueError):
...
def build_segments(chapters, only=None, ambient=True, modes=MODES, reuse=False,
    ...
    """
    manifest = {
        "overlap": OVERLAP, "dir": AUDIO_URL, "voice": VOICE,
...
def write_segment_sidecars(manifest):
    with open(os.path.join(HERE, CHAPTERS_TXT), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(HERE, MANIFEST_STEM + ".json"), "w",
              encoding="utf-8") as f:
        json.dump(manifest, f, ensure_ascii=False, indent=1)
    with open(os.path.join(HERE, MANIFEST_STEM + ".js"), "w",
              encoding="utf-8") as f:
        ...
        f.write(f"window.{MANIFEST_GLOBAL} = ")
