/* Shared header, progress tracking (localStorage), depth-mode toggle, and
   dashboard rendering for the CI/CD & Containers Course. Same local-first design as
   ...
   (tables cicd_progress / cicd_notes). */

const PROGRESS_KEY = 'cicd-progress-v1';
...
async function syncModuleToCloud(id, done) {
  ...
  if (!session) return;
  try {
    await sb.from('cicd_progress').upsert({
    ...
    console.warn('cicd_progress sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeProgress() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('cicd_progress').select('module_id, completed').eq('completed', true);
  if (error) {
    console.warn('cicd_progress pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('cicd:progress-synced'));
}

/* Per-lesson free-text notes — newer updated_at wins on merge. */
const NOTES_KEY = 'cicd-notes-v1';
...
async function syncNoteToCloud(id, text, updated_at) {
  ...
  if (!session) return;
  try {
    await sb.from('cicd_notes').upsert({
      user_id: session.user.id,
      module_id: id,
      note: text,
      updated_at,
    ...
    console.warn('cicd_notes sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeNotes() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('cicd_notes').select('module_id, note, updated_at');
  if (error) {
    console.warn('cicd_notes pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('cicd:notes-synced'));
}
...
function setDepthMode(mode) {
  ...
  window.dispatchEvent(new CustomEvent('cicd:depth-changed', { detail: { mode: getDepthMode() } }));
}
