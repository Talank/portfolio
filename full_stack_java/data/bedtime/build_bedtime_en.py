#!/usr/bin/env python3
"""Build the English edition of "निद्राको लग पोज" — the Full-Stack Java voyage.

Same voyage as the Nepali one: the same twenty-eight chapters in the same
order, the same @algo method repeated three times, the same tiers, the same
...
The Nepali edition of this voyage reads in ne-NP-SagarNeural, a male voice,
rather than the Hemkala the other two Nepali voyages use. None of that carries
over: the English cast is the shared nine-voice one imported from the DSA
English profile, so a listener who plays the DSA voyage and then this one does
not meet a different Robin.
...
_ne = _load(os.path.join(HERE, "build_bedtime.py"), "java_bedtime_ne_profile")
E = _ne.E
...
_dsa_en = _load(os.path.join(ENGINE_DIR, "build_bedtime_en.py"),
                "dsa_bedtime_en_profile")

java_words_en = _load(os.path.join(HERE, "java_words_en.py"), "java_words_en")
...
E.TITLE = "The Log Pose of Sleep"
E.SUBTITLE = "Full-Stack Java Bedtime Voyage — English"
E.MONOLITH_NAME = "java-bedtime-en-full"
...
# ---------------------------------------------------------------------------
#
# Replaces the Nepali profile's version outright. That one runs the whole Java
# vocabulary through java_words.to_devanagari() for a Nepali voice, and there is
# no Nepali voice in this edition. All that is left is turning the initialisms
# an English voice mangles into letters.
def to_speakable(text, space_lists=True):
    text = java_words_en.to_spoken(text)
