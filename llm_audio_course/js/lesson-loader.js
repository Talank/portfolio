...
function renderLesson(id) {
  ...
  document.title = `${data.title} — Local Voice Lab`;
  ...
  root.innerHTML = `
    ...
    ${techHtml}
    ${codeHtml}
    ${data.lab ? '<h2>Code Lab — build the piece</h2><div id="lab-container"></div>' : ''}
  ...
  if (data.deepDive) {
    ...
    window.addEventListener('localvoice:depth-changed', () => {
      renderDeepDive(document.getElementById('deepdive-container'), data.deepDive);
    });
  }
  ...
  window.addEventListener('localvoice:progress-synced', () => { check.checked = isComplete(id); });
  ...
  window.addEventListener('localvoice:notes-synced', () => {
