/* Shared header, progress tracking (localStorage), depth-mode toggle, and
   dashboard rendering for the Local Voice Lab. Same local-first design as
   linux_course/full_stack_java: localStorage is the always-available source
   of truth; Supabase sync is optional and additive for signed-in users
   (tables localvoice_progress / localvoice_notes). */

const PROGRESS_KEY = 'localvoice-progress-v1';
...
async function syncModuleToCloud(id, done) {
  ...
  if (!session) return;
  try {
    await sb.from('localvoice_progress').upsert({
    ...
    console.warn('localvoice_progress sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeProgress() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('localvoice_progress').select('module_id, completed').eq('completed', true);
  if (error) {
    console.warn('localvoice_progress pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('localvoice:progress-synced'));
}

/* Per-lesson free-text notes — newer updated_at wins on merge. */
const NOTES_KEY = 'localvoice-notes-v1';
...
async function syncNoteToCloud(id, text, updated_at) {
  ...
  if (!session) return;
  try {
    await sb.from('localvoice_notes').upsert({
      user_id: session.user.id,
      module_id: id,
      note: text,
      updated_at,
    ...
    console.warn('localvoice_notes sync failed (offline or table missing?)', e);
  }
}
...
async function pullAndMergeNotes() {
  ...
  const session = await AuthUI.getSession();
  if (!session) return;
  const { data, error } = await sb.from('localvoice_notes').select('module_id, note, updated_at');
  if (error) {
    console.warn('localvoice_notes pull failed (offline or table missing?)', error);
    return;
  }
  const cloud = data || [];
  ...
  window.dispatchEvent(new CustomEvent('localvoice:notes-synced'));
}
...
const DEPTH_KEY = 'localvoice-depth-v1';
...
function setDepthMode(mode) {
  ...
  window.dispatchEvent(new CustomEvent('localvoice:depth-changed', { detail: { mode: getDepthMode() } }));
}
...
function buildHeader(active) {
  ...
  el.innerHTML = `
    <header class="site">
      <div class="bar">
        <a class="brand" href="index.html">Local Voice <span>Lab</span></a>
        <nav>${nav}</nav>
...
function timeLeftLabel(doneIds) {
  ...
  return remaining === 0 ? 'All modules complete — go bind your keys with confidence' : `~${h ? h + 'h ' : ''}${m}m of study left`;
}
